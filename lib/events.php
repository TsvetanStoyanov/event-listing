...
class events 
    ...
    function gps($date) 
        {
        // map() : gps event for the selected date
        // PARAM $date : date
    ...
    function lng($date) 
        {
        // map() : lng event for the selected map point
        // PARAM $date : date
        $sql = "SELECT * FROM `markers` WHERE `date` = ?";
        $evt = $this->fetch($sql, [$date]);
    ...
    function link($date) 
        {
        // map() : link event for the selected link
        // PARAM $date : date
        $sql = "SELECT * FROM `markers` WHERE `date` = ?";
        $evt = $this->fetch($sql, [$date]);
        
        $link_result = $evt[0]['address'];
        return count($evt) == 0 ? false : $link_result;
        }
    ...
    function get_range($start, $end) 
        {
        // get_range() : get all events in between selected date range
    ...
    function save($date, $details, $map, $link, $gps, $lng)
        {
        // save() : create/update event on specified date
        // PARAM $date : date
        //       $details : event details
    
        $sql = "REPLACE INTO `events_list` (`date`, `details`, `map`) 
                                VALUES (?, ?, ?)";
        ...
        $sql2 = "REPLACE INTO `markers` (`date`, `address`, `lat`, `lng`, `type`) 
                            VALUES (?, ?, ?, ?, ?)";
        $result[1] = $this->exec($sql2, [$date, $link, $lat, $lng, $map]);
